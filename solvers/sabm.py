...
class SABM(OdeSolver):
    """Semi-explicit and semi-implicit method based on Adams-Bashforth-Moulton formulas [1]_.
    ...
    This class implements a varying step size which depends on the error estimation.
    The strategy used for the error is the difference between the predictor and the corrector.
    ...
    fun : callable
        ...
        two option for the signature of fun : It can be ``fun(t, y, x)`` to solve DAE. 
        Here ``t`` is a scalar, ``y`` is a ndarray with shape (n,) and ``x`` is 
        ndarray with shape (m,; then ``fun`` must return array_like with shape (n,). 
        Alternatively it can be ``fun(t,y)`` to solve an ODE.
    t0 : float
        Initial time.
    ...
    gun : callable, optional
        ...
        Otherwise, it is the right-hand side of the system for the algebraic variables.
        The calling signature is ``gun(t, y, x)``. Here ``t`` is a scalar, ``y`` 
        is a ndarray with shape (n,), and ``x`` is a ndarray with shape (m,); 
        then ``gun`` must return array_like with shape (m,).
    ...
    rtol : float, optional
        Relative tolerance. This tolerance is use to solve the algebraic equations
        with fsolve. Default is the default relative tolerance use by fsolve.
    atol : float, optional
        Absolute tolerance. The step will terminate if the absolute error 
        between the predictor and the corrector for each variable is at most atol. 
    ...
    """
    def __init__(self, fun, t0, y0, t_bound, gun=None, mode="Explicit", order=2, num_diff=None,  
                 max_step=np.inf, rtol=1.49012e-08, atol=1e-6, vectorized=False, first_step=None, **extraneous):
        global nbr_f, nbr_g
        nbr_f = 0
        nbr_g = 0
        ...
        if self.ode:
            ...
            def f(t, x): return np.concatenate((fun(t, x[:num_diff], x[num_diff:]), gun(
                    t, x[:num_diff], x[num_diff:])), axis=None)
    ...
    def _step_impl(self):
        ...
        h = self.h_abs
        i = np.argmax(error/y_new)
        if error[i] < 1e-14:
            h_new = min([3/2 * h, self.max_step])

        else:
            h_hat = h*(self.atol[i]/error[i])**(1/(self.p+1))
            if h_hat > h:
        ...
        h_new = min(h_new, abs(self.t_bound - (self.t+self.h_abs*self.direction)))
    ...
    def _dense_output_impl(self):
        return SABMDenseOutput(self.t_old, self.t, self.y, np.array([self.prev_f_y[-2], self.prev_f_y[-1]]), self.num_diff, self.g, self.ode)
    ...
    def Explicit_Adams_Bashforth(self):
        ...
        for i in range(0, p):
            y_p += h*self.direction*B[i]*self.prev_f_y[p-i-1]
    ...
    def Semi_Explicit_Adams_Bashforth_Moulton(self):
        ...
        for i in range(self.num_diff):
            ...
            y_new[i] += h*direction*M[0] * \
                self.f(self.t+h*direction, P, x_p)[i]
            for j in range(1, p):
                y_new[i] += h*direction*M[j]*self.prev_f_y[p-j][i]
