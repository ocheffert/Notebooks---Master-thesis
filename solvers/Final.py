...
class SABMScipy(OdeSolver):
    ...
    def _step_impl(self):
        ...
        self.nfev = int(self.nfev + self.factor) 
    ...
    def _dense_output_impl(self):
        return SABMDenseOutput(self.t_old, self.t, self.y, np.array([self.prev_f_y[-2], self.prev_f_y[-1]]), self.num_diff, self.g)
    ...
    def Explicit_Adams_Bashforth(self):
        ...
        for i in range(0, p):
            y_p += h*direction*B[i]*self.prev_f_y[p-i-1]

        def equations(z):
            return self.g(self.t + h * self.direction, y_p, z)

        (x_p, dic, flag, message) = fsolve(equations, x0=self.y[self.num_diff:], xtol=self.rtol, full_output=1)
        if flag != 1:
            return (False, "Solving the constraints in predictor fail to converge.")
        self.nfev += (1-self.factor)*dic["nfev"]
        return (y_p, x_p)
    ...
    def Semi_Implicit_Adams_Bashforth_Moulton(self):
        ...
        for i in range(self.num_diff):
            def equations(z):
                ...
                return z - res
            (y_new[i], dic, flag, message) = fsolve(equations, x0=y_p[i], xtol=self.rtol, full_output=1)
            self.nfev += self.factor*dic["nfev"]
            if flag != 1:
                error = self.atol + 1
            else:
                error = abs(y_new[i] - y_p[i])
...
class SABMDenseOutput(DenseOutput):
    def __init__(self, t_old, t, y, fy, num_diff, g):
    ...
    def _call_impl(self, t):
        ...
        if t.ndim == 0:
            ...
            sol[:num_diff,:] = np.outer(a,t**2) + np.outer(b,t) + np.outer(c,np.ones_like(t))
