...
from scipy.sparse import issparse, csc_matrix, eye, diags
...
def solve_bdf_system(fun, t_new, y_predict, c, psi, LU, solve_lu, scale, tol, mass, rowScale):
    ...
    for k in range(NEWTON_MAXITER):
        ...
        # rhs = c * f - mass.dot(psi + d)

        # rhs = rhs.multiply(rowScale)
        if rowScale is not None:
            rhs = c * f - mass.dot(psi + d)
            rhs = np.multiply(rhs, rowScale)
            # print('anormal')
        else:
            rhs = c * f - psi - d
        dy = solve_lu(LU, rhs)
...
class BDFModified(OdeSolver):
    ...
    def __init__(self, fun, t0, y0, t_bound, max_step=np.inf,
        ...
        super(BDFModified, self).__init__(fun, t0, y0, t_bound, vectorized,
        ...
        if mass is not None:
            self.DAE = True
            if issparse(self.J):
                self.mass = csc_matrix(mass)
            elif issparse(mass):
                self.mass = mass.todense()
            else:  # J et mass dense ok
                self.mass = mass
        else:
            self.DAE = False
            self.mass = I
        ...
        while not step_accepted:
            ...
            while not converged:
                if LU is None:
                    ...
                    Miter = self.mass - c * J
                    if self.DAE:
                        rowScale = np.reciprocal(abs(Miter).max(axis=1))   # check for sparse
                        if issparse(J):
                            Miter = (diags(rowScale)).dot(Miter)
                        else:
                            Miter = (np.diag(rowScale)).dot(Miter)
                    LU = self.lu(Miter)

                converged, n_iter, y_new, d = solve_bdf_system(
                    self.fun, t_new, y_predict, c, psi, LU, self.solve_lu,
                    scale, self.newton_tol, self.mass, rowScale)
